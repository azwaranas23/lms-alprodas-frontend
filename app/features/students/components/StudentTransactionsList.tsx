...
import {
  TransactionCard,
  type Transaction,
} from "~/components/organisms/TransactionCard";
...
export function StudentTransactionsList() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: [
      ...QUERY_KEYS.transactions.student,
      currentPage,
      searchQuery,
      itemsPerPage,
    ],
    queryFn: () =>
      studentTransactionsService.getTransactions({
        page: currentPage,
        limit: itemsPerPage,
  ...
  const currentTransactions: Transaction[] = transactions.map(
    (transaction: StudentTransaction) => ({
      ...
      studentEmail: "instructor@alprodas.com",
      studentAvatar: getAvatarSrc(undefined, "Course Instructor"),
      status: transaction.status.toLowerCase() as "paid" | "pending" | "failed",
    })
  );

  const handleViewDetails = (transactionId: string) => {
    navigate(`/dashboard/student/transactions/${transactionId}`);
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-brand-light text-sm font-normal">
                View and track your course purchase history
              </p>
            </div>
          </div>
