...
export default function AddLesson() {
  ...
  const { data: existingLessonsData, isLoading: existingLessonsLoading } =
    useQuery({
      queryKey: [...QUERY_KEYS.lessons, "section", defaultSection?.id],
      queryFn: () => lessonsService.getLessonsBySection(defaultSection!.id),
      enabled: !!defaultSection?.id,
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Use first section if available, otherwise use hardcoded ID
    ...
    <PermissionRoute requiredPermission="courses.read">
      <Layout>
        <Header
          ...
          subtitle="Create lesson content for Section No. 1 - Warming Up"
          backButton={{
